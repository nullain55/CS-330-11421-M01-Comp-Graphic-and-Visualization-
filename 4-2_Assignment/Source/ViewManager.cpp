...
{
	//Thomas Martin 4-2 Assignment: Basic Camera Movement
	//debug for mouse scroll
	//ensure the callback is being triggered when mouse scroll.	
	printf("Scroll Callback Triggered: xOffset = %f, yOffset = %f\n", xOffset, yOffset);
	if (g_pCamera) {
		g_pCamera->ProcessMouseScroll(static_cast<float>(yOffset));
		printf("Camera MovementSpeed: %f\n", g_pCamera->MovementSpeed);
		std::cout << "Scroll Triggered " << std::endl;
	}
}
/***********************************************************
 *  CreateDisplayWindow()
...
void ViewManager::ProcessKeyboardEvents()
{
	...
	if (glfwGetKey(m_pWindow, GLFW_KEY_H) == GLFW_PRESS && glfwGetKey(m_pWindow, GLFW_KEY_LEFT_SHIFT) == GLFW_PRESS)
	{
		std::cout << "Hello World "; 
	}
	// Toggle orthographic projection with 'O' key
	if (glfwGetKey(m_pWindow, GLFW_KEY_O) == GLFW_PRESS) 
	{
		bOrthographicProjection = true;
	}
	// Toggle perspective projection with 'P' key
